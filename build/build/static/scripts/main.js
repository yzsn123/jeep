...
function ajax(obj) {
  ...
  if (obj.type == 'get' || obj.type == 'GET') {
    ...
    xhr.send(null);
      ...
      xhr.open('post', obj.url, true); //post请求要在send前设置http请求头，作用是模拟表单post来传递参数
...
var _loop = function _loop(i) {
  carHeadLis[i].onmouseover = function () {
    ...
    var _char = 'nav-cars-news-';
    headImg.src = url + _char + imgS.replace('png', 'jpg');
  };
...
for (var i = 0; i < carHeadLis.length; i++) {
  _loop(i);
} //点击预约试驾或jeep商城显示商品页面
...
var headLoginDiv = document.querySelector('.head-login div'); // console.log(headLoginImg);

judgeUser(); //判断用户登录了没
function judgeUser() {
  if (getCookie('user')) {
    // headLoginEm.style.display = 'block';
    headLoginEm.innerText = "hello"; // headLoginImg.style.display = 'block';
...
if(document.title == "专业SUV再进化"){
  disHead();
  disSuv();
}
else if(document.title == "Jeep Life"){
  disHead();
  disLife();
}
else if(document.title == "Jeep商城"){
  disShop();
}
else if(document.title == "Jeep首页"){
  disHead();
  disIn();
}
else if(document.title == "登录/注册"){
  disHead();
  disLogin();
}
else if(document.title == "我的订单"){
  disBuy();
}
else if(document.title == "购物车"){
  disCart();
}
//历史车型年份跳转
function disLife(){
...
var _loop = function _loop(_i) {
  lifeNav[_i].onclick = function () {
    jumpA(lifeNav[_i], document.documentElement);
  };
};

for (var _i = 0; _i < lifeNav.length; _i++) {
  _loop(_i);
} //滚动条跳转
...
opaMove(clubD, clubL, clubR, clubNum); //轮播图
...
function disLogin(){
  ...
  var logRem = document.querySelector('.log-rem'); //第一个按钮的点击事件
  
  submit1.onclick = function () {
    flag = judgeLogin();
    var regPhoneV = regPhone.value;
    var regPsdV = regPsd.value;
    var regAgainV = regAgain.value; //如果当前是注册状态
    ...
    if (flag == '2') {
      if (judgeTel(regPhoneV)) {
        if (regPsdV == regAgainV && regPsdV != "") {
          ajax({
            url: 'reg.php',
            data: "phone=".concat(regPhoneV, "&psd=").concat(regPsdV),
            type: 'post',
    ...
    var logPsdV = logPsd.value; //如果当前是登录状态
    ...
    if (flag == '1') {
      if (judgeTel(logPhoneV)) {
        if (logPsdV != "") {
          ajax({
            url: 'login.php',
            data: "phone=".concat(logPhoneV, "&psd=").concat(logPsdV),
            type: 'post',
            succeed: function succeed(data) {
              var json = JSON.parse(data);
              loginH3.innerText = json.msg;
...
function disShop(){
  ...
  for (var _i = 0; _i < shopLis.length; _i++) {
    shopLis[_i].onclick = function () {
  ...
  shopCart.onclick = function () {
    if (getCookie('user')) {
      ...
      ajax({
        url: 'shop.php',
        data: "user=".concat(phone, "&code=").concat(code),
        type: 'POST',
        succeed: function succeed(data) {
          console.log(data);
  ...
  shopBuy.onclick = function () {
    if (getCookie('user')) {
      ...
      ajax({
        url: 'buy.php',
        data: "phone=".concat(phone, "&code=").concat(code, "&buy=buy"),
        type: 'POST',
        succeed: function succeed(data) {
...
function disSuv(){
  ...
  var suvImgs = document.querySelectorAll('.detail-img img'); //点击关闭按钮
...
function disBuy(){
  ...
  var buyWrap = document.querySelector('.buy-wrap'); //如果用户登录了
  
  if (getCookie('user')) {
    var buyPhone = getCookie('user'); //通过buy.php获取数据库里的code
  
    ajax({
      url: 'buy.php',
      data: "phone=".concat(buyPhone, "&buy=see"),
      //传送一个buy状态，buy为购买，see为查看, del为删除
      type: 'post',
      succeed: function succeed(data) {
        var buyJson = JSON.parse(data); // console.log(buyJson);
        //通过从php里得到的数据跟json数据对比，然后渲染
  
        ajax({
          url: '../static/data/data.json',
          type: 'post',
          succeed: function succeed(d) {
            var arr = [];
            var d = JSON.parse(d);
            ...
            for (var k in buyJson) {
              for (var j in d) {
                if (buyJson[k].code == d[j].code) {
                  ...
                  li.innerHTML = "\n                                <img src=\"".concat(d[j].imgUrl, "\" alt=\"\">\n                                <h3>").concat(d[j].title, "</h3>\n                                <p>").concat(d[j].detail, "</p>\n                                <h4>").concat(d[j].price, "</h4>\n                                <button code=\"").concat(d[j].code, "\">\u5220\u9664</button>");
                  buy.appendChild(li);
                }
              }
            }
            ...
            if (arr.length > 0) {
              ...
              var _loop = function _loop(i) {
                buyDel[i].onclick = function () {
                  ...
                  ajax({
                    url: 'buy.php',
                    data: "phone=".concat(buyPhone, "&buy=del&code=").concat(delCode),
                    type: 'post',
...
if (getCookie('user')) {
  var cartPhone = getCookie('user'); //通过cart.php获取数据库里的code

  ajax({
    url: 'cart.php',
    data: "phone=".concat(cartPhone),
    type: 'post',
    succeed: function succeed(data) {
      ...
      ajax({
        ...
        succeed: function succeed(d) {
          ...
          for (var k in cartJson) {
            for (var j in d) {
              if (cartJson[k].code == d[j].code) {
                obj["".concat(cartJson[k].code)] = d[j];
              }
            }
          }
          ...
          if (obj) {
            ...
            for (var i in obj) {
              var li = document.createElement('li');
              li.innerHTML = "\n                        <img src=\"".concat(obj[i].imgUrl, "\" alt=\"\">\n                        <h3>").concat(obj[i].title, "</h3>\n                        <p>").concat(obj[i].detail, "</p>\n                        <h4>").concat(obj[i].price, "</h4>\n                        <span>\u7ACB\u5373\u8D2D\u4E70</span>\n                        <button code=\"").concat(obj[i].code, "\">\u5220\u9664</button>");
              cart.appendChild(li);
            }
            ...
            var _loop = function _loop(_i) {
              cartDel[_i].onclick = function () {
                // console.log(this.getAttribute('code'))
                var delCode = this.getAttribute('code');
                ajax({
                  url: 'delet.php',
                  data: "phone=".concat(cartPhone, "&code=").concat(delCode),
                  type: 'post',
                  succeed: function succeed() {
                    cart.removeChild(cartDel[_i].parentNode);
            ...
            for (var _i = 0; _i < cartDel.length; _i++) {
              _loop(_i);
            }
            ...
            var _loop2 = function _loop2(_i2) {
              cartSpan[_i2].onclick = function () {
                var buyCode = cartDel[_i2].getAttribute('code');

                ajax({
                  url: 'buy.php',
                  data: "phone=".concat(cartPhone, "&code=").concat(buyCode, "&buy=buy"),
                  type: 'post',
                  succeed: function succeed(data) {
            ...
            for (var _i2 = 0; _i2 < cartDel.length; _i2++) {
              _loop2(_i2);
            }
          } else {
